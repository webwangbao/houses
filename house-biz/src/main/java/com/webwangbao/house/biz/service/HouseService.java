...
import com.webwangbao.house.biz.mapper.HouseMapper;
import com.webwangbao.house.common.constants.HouseUserType;
...
import com.webwangbao.house.common.utils.BeanHelper;
/**
 * @author :wangbao02
 * @desc:房产操作业务逻辑层
 * @date:2018/04/18
...
public class HouseService {
	...
	public PageData<House> queryHouseNew(House query, PageParams pageParams) {
		List<House> houses = Lists.newArrayList();
		if (!Strings.isNullOrEmpty(query.getName())) {
			Community community = new Community();
			community.setName(query.getName());
		...
		//添加图片服务器地址前缀
		houses = queryAndSetImg1(query,pageParams);
		Long count = houseMapper.selectPageCount(query);
		return PageData.buildPage(houses, count, pageParams.getPageSize(), pageParams.getPageNum());
	}
	...
	public List<House> queryAndSetImg1(House query, PageParams pageParams) {
		List<House> houses =   houseMapper.getOwnHouseList(query, pageParams);
		houses.forEach(h ->{
			h.setFirstImg(imgPrefix + h.getFirstImg());
			h.setImageList(h.getImageList().stream().map(img -> imgPrefix + img).collect(Collectors.toList()));
			h.setFloorPlanList(h.getFloorPlanList().stream().map(img -> imgPrefix + img).collect(Collectors.toList()));
	...
	public void addHouse(House house, User user) {
		if (CollectionUtils.isNotEmpty(house.getHouseFiles())) {
			List<String> imsList = fileService.getImgPaths(house.getHouseFiles());
			String images = Joiner.on(",").join(imsList);
		    house.setImages(images);
		}
		if (CollectionUtils.isNotEmpty(house.getFloorPlanFiles())) {
			List<String> imgList = fileService.getImgPaths(house.getFloorPlanFiles());
			String images = Joiner.on(",").join(imgList);
	...
	public void upHouse(Long id, Long userId, HouseUserType type) {
		houseMapper.upHouse(id);
	}
